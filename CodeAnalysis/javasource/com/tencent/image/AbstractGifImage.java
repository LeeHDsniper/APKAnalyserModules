...
public abstract class AbstractGifImage
{
  ...
  public void attachDrawable(GifDrawable paramGifDrawable)
  {
    if (paramGifDrawable != null) {
      ...
      {
        if (!mDrawableList.contains(paramGifDrawable)) {
          mDrawableList.add(new WeakReference(paramGifDrawable));
        }
        return;
      }
    }
  ...
  public void detachDrawable(GifDrawable paramGifDrawable)
  {
    if (paramGifDrawable != null)
    {
      ...
      {
        paramGifDrawable = mDrawableList;
    ...
    extends ArgumentsRunnable<WeakReference<AbstractGifImage>>
  {
    ...
    protected void submit()
    {
      ...
      if ((lastRefreshTime == 0L) || (l - lastRefreshTime > DELAY))
      {
        run();
        lastRefreshTime = l;
        return;
      }
      AbstractGifImage.sUIThreadHandler.postDelayed(this, DELAY - (l - lastRefreshTime));
    }
  }
}
