  ...
  extends AbstractGifImage
{
    ...
    throws IOException
  {
    ...
    mSrcGifFile = paramFile.getAbsolutePath();
    mIsEmosmFile = paramBoolean2;
    if ((!paramFile.exists()) && (QLog.isColorLevel())) {
      QLog.e("NativeGifImage", 2, mSrcGifFile + " doesn't exist");
    }
    ...
    if (mIsGIFEngineAvaliable) {
      mGifFilePtr = nativeOpenFile(mMetaData, paramFile.getPath(), mCurrentFrameBitmap, paramBoolean2);
    }
    for (;;)
    {
      mDefaultRoundCorner = paramFloat;
      init(paramBoolean1);
      return;
      label257:
      f1 = f2;
      break;
      mMetaData[WIDTH_INDEX] = localRect.width();
      mMetaData[HEIGHT_INDEX] = localRect.height();
      mMetaData[FRAME_COUNT_INDEX] = 1;
      mMetaData[ERRCODE_INDEX] = 0;
      mMetaData[POST_INVALIDATION_TIME_INDEX] = Integer.MAX_VALUE;
      mMetaData[CURRENT_FRAMEINDEX_INDEX] = -1;
      mMetaData[CURRENT_LOOP_INDEX] = -1;
    }
  }
    ...
    throws IOException
  {
    ...
    for (;;)
    {
      ...
      arrayOfInt[WIDTH_INDEX] = outWidth;
      arrayOfInt[HEIGHT_INDEX] = outHeight;
    }
  }
  ...
  private void init(boolean paramBoolean)
  {
    ...
    try
    {
      mFirstFrameBitmap = mCurrentFrameBitmap.copy(mCurrentFrameBitmap.getConfig(), false);
      return;
    }
  ...
  private void initCurrentFrameBitmap()
  {
    ...
    try
    {
      mCurrentFrameBitmap = Bitmap.createBitmap(mReqWidth, mReqHeight, mCurrentConfig);
      if (mReqWidth * mReqHeight > IMAGE_SIZE_DISABLE_DOUBLE_BUFFER) {}
    }
    catch (OutOfMemoryError localOutOfMemoryError1)
    {
      for (;;)
      {
        try
        {
          mCurrentFrameBitmapBuffer = Bitmap.createBitmap(mReqWidth, mReqHeight, mCurrentConfig);
          return;
        }
        ...
        if (mCurrentConfig == Bitmap.Config.ARGB_8888)
        {
          try
          {
            mCurrentConfig = Bitmap.Config.ARGB_4444;
            mCurrentFrameBitmap = Bitmap.createBitmap(mReqWidth, mReqHeight, mCurrentConfig);
          }
  ...
  public void draw(Canvas paramCanvas, Rect paramRect, Paint paramPaint, boolean paramBoolean)
  {
    ...
    if ((mMetaData[FRAME_COUNT_INDEX] <= 1) || ((!paramBoolean) && (mCurrentFrameBitmap != null))) {
      paramCanvas.drawBitmap(mCurrentFrameBitmap, null, paramRect, paramPaint);
    }
    do
    {
      return;
  ...
  protected void executeNewTask()
  {
    ...
    if (mDecodeNextFrameEnd)
    {
      ...
      l2 = mMetaData[POST_INVALIDATION_TIME_INDEX];
    }
    try
    {
  ...
  public int getByteSize()
  {
    ...
    try
    {
      l1 = nativeGetAllocationByteCount(mGifFilePtr);
      l1 = 0L + l1;
    }
    catch (UnsatisfiedLinkError localUnsatisfiedLinkError)
    {
    ...
    return (int)(l1 + Utils.getBitmapSize(mCurrentFrameBitmap) + Utils.getBitmapSize(mFirstFrameBitmap));
  }
  
  public NativeGifIOException.NativeGifError getError()
  {
    return NativeGifIOException.NativeGifError.fromCode(mMetaData[ERRCODE_INDEX]);
  }
  
  public int getHeight()
  {
    if (mCurrentFrameBitmap != null) {
      return mCurrentFrameBitmap.getHeight();
    }
    return 0;
  }
  ...
  protected void getNextFrame()
  {
    for (;;)
    {
      try
      {
        if (mIsGIFEngineAvaliable)
        {
          if (mCurrentFrameBitmapBuffer != null)
          {
            nativeSeekToNextFrame(mCurrentFrameBitmapBuffer, mGifFilePtr, mMetaData, sequence);
            return;
          }
          nativeSeekToNextFrame(mCurrentFrameBitmap, mGifFilePtr, mMetaData, sequence);
          continue;
        }
        try
        {
          mCurrentFrameBitmap = BitmapFactory.decodeFile(mSrcGifFile);
          mCurrentFrameBitmap = Bitmap.createScaledBitmap(mCurrentFrameBitmap, mReqWidth, mReqHeight, true);
        }
        catch (OutOfMemoryError localOutOfMemoryError) {}
      }
      finally {}
  ...
  {
    if (mCurrentFrameBitmap != null) {
      return mCurrentFrameBitmap.getWidth();
    }
    return 0;
  }
  ...
  public void reset()
  {
    ...
    mCurrentLoop = -1;
    if (mIsGIFEngineAvaliable) {
      nativeReset(mGifFilePtr);
    }
  }
  
  public String toString()
  {
    return String.format(Locale.US, "Size: %dx%d, %d frames, error: %d", new Object[] { Integer.valueOf(mMetaData[0]), Integer.valueOf(mMetaData[1]), Integer.valueOf(mMetaData[2]), Integer.valueOf(mMetaData[3]) });
  }
    ...
    extends AsyncTask<Void, Void, Void>
  {
    ...
    {
      getNextFrame();
      if (mMetaData[NativeGifImage.FRAME_COUNT_INDEX] > 1)
      {
        ...
        if (l < mNextFrameTime)
        {
          AbstractGifImage.sUIThreadHandler.postDelayed(paramVarArgs, mNextFrameTime - l);
          return null;
        }
