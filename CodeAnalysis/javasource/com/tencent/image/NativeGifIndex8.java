  ...
  extends AbstractGifImage
{
    ...
    throws IOException
  {
    ...
    mCacheFirstFrame = paramBoolean1;
    if (!paramFile.exists())
    {
      if (QLog.isColorLevel()) {
        QLog.e("NativeGifIndex8", 2, mSrcGifFile + " doesn't exist");
      }
      return;
    }
    mExtData = new int[2];
    ...
    {
      mGifHandler = nativeInit(mSrcGifFile, paramBoolean2);
      initSize(paramInt1, paramInt2);
      nativeSetDestSize(mGifHandler, mReqWidth, mReqHeight);
      nativeSetLoopCount(mGifHandler, -1);
      mFrameNumber = nativeGetFrameNum(mGifHandler);
      mExtData[0] = 0;
    }
    for (;;)
    {
  ...
  private void init(boolean paramBoolean)
  {
    ...
    try
    {
      mFirstFrameBitmap = mCurrentFrameBitmap;
      return;
    }
  ...
  protected void draw(Canvas paramCanvas, Rect paramRect, Paint paramPaint, boolean paramBoolean)
  {
    ...
    if ((mFrameNumber <= 1) || (!paramBoolean)) {
      if ((mCurrentFrameBitmap != null) && (!mCurrentFrameBitmap.isRecycled())) {
    ...
    do
    {
      ...
      {
        paramCanvas.drawBitmap(mCurrentFrameBitmap, null, paramRect, paramPaint);
        if ((mLastBitmap != null) && (!mLastBitmap.isRecycled()) && (!mIsFirstBitmap))
        {
          mLastBitmap.recycle();
          mIsFirstBitmap = false;
        }
        if ((mCurrentFrameBitmap != null) && (!mCurrentFrameBitmap.isRecycled())) {
          mLastBitmap = mCurrentFrameBitmap;
        }
        if (!sPaused)
        {
          executeNewTask();
  ...
  {
    if (mIsGIFEngineAvaliable) {}
    return (int)(0L + Utils.getBitmapSize(mCurrentFrameBitmap) + Utils.getBitmapSize(mFirstFrameBitmap));
  }
  
  public NativeGifIOException.NativeGifError getError()
  {
    return NativeGifIOException.NativeGifError.fromCode(mExtData[1]);
  }
  
  public int getHeight()
  {
    if (mGifHandler != 0) {
      return nativeGetHeight(mGifHandler);
    }
    return 0;
  }
  ...
  public Rect getImageSize()
  {
    int j;
    if (mIsGIFEngineAvaliable) {
      j = nativeGetWidth(mGifHandler);
    }
    BitmapFactory.Options localOptions;
    for (int i = nativeGetHeight(mGifHandler);; i = outHeight)
    {
      return new Rect(0, 0, j, i);
      localOptions = new BitmapFactory.Options();
      inJustDecodeBounds = true;
      BitmapFactory.decodeFile(mSrcGifFile, localOptions);
      j = outWidth;
    }
  }
  ...
  protected void getNextFrame()
  {
    ...
    //   104	11	3	localIllegalArgumentException	IllegalArgumentException
  ...
  {
    if (mGifHandler != 0) {
      return nativeGetWidth(mGifHandler);
    }
    return 0;
  }
