...
public class MsfHttpRespParse
{
  ...
  public MsfHttpRespParse(MsfSocketInputBuffer paramMsfSocketInputBuffer, LineParser paramLineParser, int paramInt1, int paramInt2)
    throws IOException
  {
    msfHttpResp = new MsfHttpResp(paramMsfSocketInputBuffer);
  ...
  {
    lineBuf.clear();
    if (msfHttpResp.getSessionBuffer().readLine(lineBuf) == -1) {
      throw new NoHttpResponseException("The target server failed to respond");
    }
    Object localObject = new ParserCursor(0, lineBuf.length());
    localObject = lineParser.parseStatusLine(lineBuf, (ParserCursor)localObject);
    msfHttpResp.setStatusline((StatusLine)localObject);
  }
    ...
    throws IOException, HttpException
  {
    ...
    {
      parseHead();
      Header[] arrayOfHeader = parseHeaders(maxHeaderCount, maxLineLen, lineParser);
      msfHttpResp.setHeaders(arrayOfHeader);
      return msfHttpResp;
    }
    ...
    throws HttpException, IOException
  {
    ...
    for (;;)
    {
      ...
      while ((msfHttpResp.getSessionBuffer().readLine(paramLineParser) == -1) || (paramLineParser.length() < 1))
      {
