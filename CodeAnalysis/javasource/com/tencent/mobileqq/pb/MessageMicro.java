  ...
  extends PBPrimitiveField<T>
{
  ...
  private final FieldMap getFieldMap()
  {
    ...
    try
    {
      ...
      _fields = ((FieldMap)((Field)localObject).get(this));
      return _fields;
    }
  ...
  protected static final class FieldMap
  {
    ...
    {
      tags = paramArrayOfInt;
      defaultValues = paramArrayOfObject;
      fields = new Field[paramArrayOfInt.length];
      int i = 0;
      for (;;)
      {
        if (i < paramArrayOfInt.length) {
          try
          {
            fields[i] = paramClass.getField(paramArrayOfString[i]);
            i += 1;
          }
      ...
      throws IllegalArgumentException, IllegalAccessException
    {
      ...
      while (i < tags.length)
      {
        ((PBField)fields[i].get(paramMessageMicro)).clear(defaultValues[i]);
        i += 1;
      }
    }
    ...
    {
      paramInt = Arrays.binarySearch(tags, paramInt);
      if (paramInt < 0) {
        return null;
      }
      ...
      throws IllegalArgumentException, IllegalAccessException
    {
      ...
      while (i < tags.length)
      {
        int k = WireFormatMicro.getTagFieldNumber(tags[i]);
        j += ((PBField)fields[i].get(paramMessageMicro)).computeSize(k);
        i += 1;
      }
      return j;
    }
      ...
      throws IOException, IllegalArgumentException, IllegalAccessException, InstantiationException
    {
      ...
      ((PBField)fields[paramInt].get(paramMessageMicro)).readFrom(paramCodedInputStreamMicro);
      return true;
    }
      ...
      throws IllegalArgumentException, IllegalAccessException, IOException
    {
      ...
      while (i < tags.length)
      {
        int j = WireFormatMicro.getTagFieldNumber(tags[i]);
        ((PBField)fields[i].get(paramMessageMicro)).writeTo(paramCodedOutputStreamMicro, j);
        i += 1;
      }
    }
  }
