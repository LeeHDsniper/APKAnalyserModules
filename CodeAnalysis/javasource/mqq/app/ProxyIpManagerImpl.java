  ...
  implements ProxyIpManager
{
  ...
  public ProxyIpManagerImpl(AppRuntime paramAppRuntime)
  {
    ...
    mMsfStore = new MsfStore();
    mMsfStore.init(BaseApplication.getContext());
    reloadCache(mApp.getAccount(), true);
  }
  
  public List<ProxyIpManager.ProxyIp> getProxyIp(int paramInt)
  {
    if (!isMobileNetWork(mApp.getApplication()))
    {
      QLog.d("ProxyIpManagerImpl", 1, "Load local proxy ip failed, uin:" + MsfSdkUtils.getShortUin(mApp.getAccount()) + " type: " + paramInt + " reason: not mobile network!");
      return null;
    }
    if ((mProxyIpCache != null) && (mProxyIpCache.size() > 0))
    {
      Object localObject1 = mProxyIpCache.iterator();
      while (((Iterator)localObject1).hasNext())
      {
        Object localObject2 = (ProxyIpList)((Iterator)localObject1).next();
        if ((localObject2 != null) && (uService_type == paramInt))
        {
          if ((vIplist != null) && (vIplist.size() > 0))
          {
            localObject1 = new ArrayList();
            localObject2 = vIplist.iterator();
            while (((Iterator)localObject2).hasNext())
            {
              ...
              type = uType;
              ip = StringUtils.getIpAddrFromInt(uIp);
              port = uPort;
              ((ArrayList)localObject1).add(localProxyIp);
            }
            QLog.d("ProxyIpManagerImpl", 1, "Load local proxy ip succ, uin: " + MsfSdkUtils.getShortUin(mApp.getAccount()) + " type: " + paramInt + " iplist: " + ((ArrayList)localObject1).toString());
            return localObject1;
          }
          QLog.d("ProxyIpManagerImpl", 1, "Load local proxy ip failed, uin: " + MsfSdkUtils.getShortUin(mApp.getAccount()) + " type: " + paramInt + " reason: ip list is null!");
          return null;
        }
      }
    }
    QLog.d("ProxyIpManagerImpl", 1, "Load local proxy ip failed, uin: " + MsfSdkUtils.getShortUin(mApp.getAccount()) + " type: " + paramInt + " reason: ip cache is null!");
    return null;
  }
  ...
  {
    if ((TextUtils.isEmpty(paramString)) || (!paramString.equals(mApp.getAccount())))
    {
      QLog.d("ProxyIpManagerImpl", 1, "Load local proxy ip failed: uin is mismatch with app account");
      return;
    }
    Object localObject = "_key_proxyip_" + mApp.getAccount();
    for (;;)
    {
      try
      {
        MsfStore.getNativeConfigStore().loadConfig(mApp.getApplication(), false);
        ...
        if ((localProxyIpChannel == null) || (vProxy_iplists == null) || (vProxy_iplists.size() <= 0)) {
          continue;
        }
        mProxyIpCache = vProxy_iplists;
      }
      ...
      mHandler.sendEmptyMessageDelayed(10000, 100L);
      return;
