...
public class EncryptedDatabase
{
  ...
  public EncryptedDatabase(Context paramContext, String paramString, int paramInt)
  {
    ...
    mHelper = new DBHelper(paramContext, paramString, paramInt);
  }
  ...
  {
    if (!tableMap.containsKey(paramString))
    {
      localObject2 = null;
      localObject1 = null;
    }
    try
    {
      localCursor = mHelper.getReadableDatabase().rawQueryWithFactory(null, "select sql from sqlite_master where type=? and name=?", new String[] { "table", paramString }, "sqlite_master");
      if (localCursor != null)
      {
        ...
        if (localCursor.moveToFirst())
        {
          ...
          localObject1 = localCursor;
          localObject2 = localCursor;
          tableMap.put(paramString, localArrayList);
        }
      }
    }
    catch (Exception localException)
    ...
    return (ArrayList)tableMap.get(paramString);
  }
  ...
  {
    mHelper.getWritableDatabase().execSQL(paramString);
  }
  ...
  public long insert(String paramString, ContentValues paramContentValues)
  {
    ...
    return mHelper.getWritableDatabase().insert(paramString, null, paramContentValues);
  }
  
  public Cursor rawQuery(String paramString, String[] paramArrayOfString)
  {
    return mHelper.getReadableDatabase().rawQuery(paramString, paramArrayOfString);
  }
    ...
    extends SQLiteOpenHelper
  {
    ...
    {
      super(paramString, encryptedFactory, paramInt);
    }
