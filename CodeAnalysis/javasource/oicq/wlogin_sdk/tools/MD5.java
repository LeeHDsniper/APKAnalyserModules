...
public class MD5
{
  ...
  private void md5Update(byte[] paramArrayOfByte, int paramInt)
  {
    ...
    if (paramInt >= m)
    {
      md5Memcpy(buffer, paramArrayOfByte, k, 0, m);
      md5Transform(buffer);
      i = m;
      while (i + 63 < paramInt)
      {
  ...
  private boolean md5Update(InputStream paramInputStream, long paramLong)
  {
    ...
    if (paramLong >= i)
    {
      ...
      try
      {
        ...
        md5Memcpy(buffer, (byte[])localObject, j, 0, i);
        md5Transform(buffer);
      }
    ...
    for (;;)
    {
      ...
      {
        paramInputStream.read(arrayOfByte);
        md5Memcpy(buffer, arrayOfByte, j, 0, arrayOfByte.length);
        return true;
      }
      catch (Exception paramInputStream)
      {
