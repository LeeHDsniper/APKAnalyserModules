...
Permission::Permission( std::string name, std::string level, std::string info, std::string description,double weight )
{
...
Manifest_Permissions::Manifest_Permissions()
{
	...
	permissions[8]		= Permission( "GET_ACCOUNTS", "normal", "查找已知帐户", "允许应用程序访问已知的电话帐户列表。" ,0.47);
	permissions[9]		= Permission( "SEND_SMS", "dangerous", "发送短信", "允许应用程序发送短信。恶意应用程序可能在没有确认发送的情况下消耗电话资费。",0.94 );
	...
	permissions[12]		= Permission( "ACCESS_COARSE_LOCATION", "dangerous", "（基于网络的）粗略定位", "访问定位源，如移动网络数据库，确定手机大概的位置。恶意应用程序可以使用这个确定您大致的地理位置。",6.11 );
	permissions[13]		= Permission( "BIND_DEVICE_ADMIN", "signature", "设备管理互动", "允许持有人向设备管理员发送指令。这并不是应用程序常见的必要权限。" ,0.94);
	...
	permissions[15]		= Permission( "WRITE_SECURE_SETTINGS", "signatureOrSystem", "修改安全系统设置", "允许应用程序修改系统的安全设置数据。不用于常见的应用程序。",0.94 );
	...
	permissions[19]		= Permission( "DEVICE_POWER", "signature", "开关机控制", "允许应用程序开关手机" ,0.47);
	...
	permissions[22]		= Permission( "ACCESS_SURFACE_FLINGER", "signature", "访问SurfaceFlinger", "允许应用程序使用surfaceflinguer低级功能。" ,0.47);
	permissions[23]		= Permission( "CHANGE_CONFIGURATION", "dangerous", "更改UI设置", "允许应用程序更改当前的配置，如时区设置或整体字体大小。",0.47 );
	...
	permissions[29]		= Permission( "ACCESS_LOCATION_EXTRA_COMMANDS", "normal", "访问额外的位置提供命令", "访问额外的位置提供命令。恶意应用程序可以使用此来干扰全球定位系统或其他位置源." ,1.88);
	
	permissions[30]		= Permission( "PERFORM_CDMA_PROVISIONING", "signatureOrSystem", "直接启动CDMA手机设置", "允许应用程序启动CDMA配置。恶意应用程序可能会启动不必要的CDMA配置" );
	

	permissions[31]		= Permission( "WRITE_APN_SETTINGS", "dangerous", "写入访问点名称设置", "允许应用程序修改APN设置，如代理和任何APN端口。" ,0.94);
	...
	permissions[37]		= Permission( "PROCESS_OUTGOING_CALLS", "dangerous", "拦截电话", "允许应用程序拨打电话或者在拨号时更改号码。恶意应用程序可以监视，重定向或阻止呼叫.",0.47 );
	...
	permissions[40]		= Permission( "MODIFY_PHONE_STATE", "signatureOrSystem", "修改手机状态", "允许应用程序控制设备的功能。拥有该权限的应用程序可以在没有通知您的情况下切换网络，打开或关闭手机上的收音机等。",0.94 );
	...
	permissions[51]		= Permission( "AUTHENTICATE_ACCOUNTS", "dangerous", "扮演帐户认证者", "允许应用程序以账户管理者的角色使用账户认证功能，包括创建帐户以及获取和设置他们的密码." ,0.94);
	...
	permissions[55]		= Permission( "MOUNT_UNMOUNT_FILESYSTEMS", "dangerous", "安装和卸载文件系统", "允许应用程序安装和卸载可移动存储的文件系统.",0.47 );
	...
	permissions[61]		= Permission( "ACCESS_WIFI_STATE", "normal", "查看Wi-Fi状态", "允许应用程序查看Wi-Fi状态的信息。",7.52 );
	
	permissions[62]		= Permission( "SET_WALLPAPER_COMPONENT", "signatureOrSystem", "", "" );
	

	permissions[63]		= Permission( "INTERNET", "dangerous", "充分访问互联网", "允许应用程序创建网络套接字。",0.47 );
	...
	permissions[70]		= Permission( "ACCESS_NETWORK_STATE", "normal", "查看网络状态", "允许应用程序查看所有网络的状态。",11.28 );
	permissions[71]		= Permission( "RECORD_AUDIO", "dangerous", "音频记录", "允许应用程序访问音频记录路径。" ,0.47);
	
	
	permissions[72]		= Permission( "READ_EXTERNAL_STORAGE", "dangerous", "读取SD卡内容", "允许应用程序读取SD卡。" );
	

	permissions[73]		= Permission( "ACCESS_MOCK_LOCATION", "dangerous", "测试的模拟位置源", "为测试创建模拟位置源。恶意应用程序可以使用此来覆盖由全球定位系统或网络供应商所提供的实际位置源的位置和状态。" ,0.94);
	permissions[82]		= Permission( "ACCESS_CHECKIN_PROPERTIES", "signatureOrSystem", "访问登录属性", "允许对登记服务上传的属性进行读/写访问。不用于普通的应用程序。",0.94);
	permissions[92]		= Permission( "CHANGE_WIFI_STATE", "dangerous", "改变Wi-Fi状态", "允许应用程序从Wi-Fi接入点连接和断开，并更改Wi-Fi网络的配置。" ,0.47);
	permissions[99]		= Permission( "BLUETOOTH_ADMIN", "dangerous", "蓝牙管理", "允许应用程序配置本地蓝牙电话，并发现和配对远程设备。" ,0.47);
	permissions[117]	= Permission( "ACCOUNT_MANAGER", "signature", "账户管理服务", "允许应用程序调用账户认证" ,0.94);
	permissions[119]	= Permission( "ACCESS_FINE_LOCATION", "dangerous", "精良 (GPS) 定位", "在可用的环境中访问精细定位源，如手机上的全球定位系统。恶意应用程序可以使用这个来确定你在哪里，同时可能会消耗更多的电池电量。",5.17 );
	permissions[122]	= Permission( "READ_SMS", "dangerous", "读取SMS 或MMS", "允许应用程序读取存储在您的手机或SIM卡上的短信。恶意应用程序可以读取您的机密信息。" ,0.47);
	permissions[132]	= Permission( "BLUETOOTH", "dangerous", "建立蓝牙连接", "允许应用程序查看本地蓝牙电话的配置，并与配对设备进行连接或接受其他配对设备的连接请求。" ,0.47);
	permissions[137]	= Permission( "READ_CONTACTS", "dangerous", "读取联系人数据", "允许应用程序读取存储在您手机上的所有联系人（地址）数据。恶意应用程序可凭此权限将数据发送给其他人。" ,0.94);
